...
class Connector(QGraphicsPolygonItem):
    ...
    def __init__(self, ct: int, pos: tuple, z_val=None, parent=None):
        ...
        self.prim_connc = []  # primary connections: holds reference and updates connection's a-pos
        self.scnd_connc = []  # secondary connections: holds reference and updates connection's b-pos
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        print("generated new temp connc")
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if self.temp_connection is not None:
            self.temp_connection.drag_line((event.scenePos().x(), event.scenePos().y()))

    def mouseReleaseEvent(self, event: QGraphicsSceneMouseEvent):
        if self.temp_connection is not None:
            print("deleted temp connc", self.temp_connection)
            scene: QGraphicsScene = self.scene()
            ...
            for i in scene.items(event.scenePos()):
                ...
                if isinstance(i, Connector):
                    ...
                    if (self.ct ^ i.ct) & 0b11 == 0b11:
                        ...
                        scene.addItem(connc)
                        self.prim_connc.append(connc)
                        i.scnd_connc.append(connc)
