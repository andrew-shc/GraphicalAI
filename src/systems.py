...
def shwCursor( self, glbl ):  # TODO
	...
	for eid, dt in zip( reversed( e_ind ), reversed( e_main ) ):
		if dt["at"]:
			...
			if glbl["chrc"]["raw"] == p.K_RIGHT and dt["cursor"] < len(dt["text"]): dt["cursor"] += 1
			elif glbl["chrc"]["raw"] == p.K_LEFT and dt["cursor"] > 0: dt["cursor"] -= 1
...
def at( self, glbl ):
	...
	at = False
	e_click = _entity_only( self, ["at"] ) # check if any other entity has clicked
	for c in e_click:
		if c["at"]: at = True
	...
	for dt in e_main:
		...
		for f in e_missing:  # creating a new entity
			if dt["field"][f] == glbl["node_typ"]["inp"]:
				...
				self.create(obj_id=dt["child"], pos=[dt["pos"][0], dt["pos"][1]+ind_u*(nd_sz+nd_pd)+nd_pd+dt["rect"][1]/4+nd_pd],
				            rect=[dt["rect"][0], 25], color=(0, 0, 0), fld_nm=f, fld_typ=dt["field"][f], fld_dt=None )
